...
public interface FacultadRepository extends JpaRepository<Facultad, Long> {
	...
	/**
	 * Obtiene las estad铆sticas de la actividad investigativa.
	 *
	 * @return las estad铆sticas: Cantidad de grupos, investigadores y centros de
	 *         investigaci贸n.
	 */
	...
	/**
	 * Obtiene el resumen general de la facultad en n煤meros la lista en cada
	 * posici贸n obtiene lo siguiente: 0 - Cantidad programas acad茅micos 1 - Cantidad
	 * doctorados 2 - Cantidad maestr铆as 3 - Cantidad especializaciones 4 . Cantidad
	 * centros de investigaci贸n 5 . Cantidad grupos de investigaci贸n 6 - Cantidad de
	 * lineas investigaci贸n 7 - Cantidad investigadores 8 - Grupos categoria A1 9 -
	...
	@Query(value = "SELECT count(*) FROM gri.facultades f join gri.programas p on f.id=p.facultades_id WHERE f.id=:facultadId AND p.nombre LIKE '%MAESTR虯%' UNION ALL SELECT count(*) FROM gri.centros WHERE id<>0 UNION ALL SELECT count(*) FROM gri.programas where id<>0 UNION ALL SELECT count(*) FROM gri.grupos WHERE id<>0 UNION ALL SELECT count(distinct(i.id)) FROM gri.investigadores i join gri.grupos_inves gi ON i.id = gi.investigadores_id join gri.grupos g ON g.id = gi.grupos_id WHERE gi.estado = 'ACTUAL'", nativeQuery = true)
